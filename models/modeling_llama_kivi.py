        ...
        **kwargs,
        ...
        if q_len == 1:
            ...
            if key_states_full.shape[-2] == self.residual_length:
                ...
                key_scale_trans = key_scale_trans_new if key_states_quant_trans is None else torch.cat([key_scale_trans, key_scale_trans_new], dim=3)
                key_mn_trans = key_mn_trans_new if key_states_quant_trans is None else torch.cat([key_mn_trans, key_mn_trans_new], dim=3)
...
class LlamaMixedAttention(LlamaKIVIAttention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: torch.Tensor = None,
        ...
        **kwargs,
        ...
        if q_len == 1:
            ...
            if value_full_length > self.residual_length:
                ...
                value_states_quant_new, scale, mn = triton_quantize_and_pack_along_last_dim(value_states_full[:, :, :1, :].contiguous(), 128, self.v_bits) # we fixed it as 128
            ...
            if value_states.shape[-2] <= self.residual_length:
                ...
                value_states_quant, value_scale, value_mn = triton_quantize_and_pack_along_last_dim(value_states_quant, 128, self.v_bits)  # we fixed it as 128
...
class LlamaKIVIModel(LlamaModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([LlamaKIVIDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
...
class LlamaKIVIForCausalLM(LlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
...
if __name__ == '__main__':
    ...
    model = LlamaKIVIForCausalLM.from_pretrained(pretrained_model, config=config, torch_dtype=torch.bfloat16).to(device='cuda')
    model.eval()
...
# CUDA_VISIBLE_DEVICES=0 python modeling_llama_kivi.py
